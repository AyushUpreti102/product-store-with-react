  ...
  return (
    <Link to={`/product/${details.category.split(' ').join('-')}/${details.id}`} style={{ textDecoration: 'none', color: 'black' }}>
      <div className="card h-100 shadow-sm" role="button">
        <img src={details.images[0]} className="card-img-top card-image" alt="product image" />
        <div className="card-body">
          <div className="clearfix mb-3">
            <span className="float-start badge rounded-pill bg-primary">{details.brand}</span>
